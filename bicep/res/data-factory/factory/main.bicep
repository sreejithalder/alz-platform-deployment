...
@description('Optional. An array of objects for the configuration of Credentials.')
...
module dataFactory_credentials 'credential/main.bicep' = [
  for (credential, index) in credentials: {
    name: '${uniqueString(deployment().name, location)}-DataFactory-Credentials-${index}'
    params: {
      dataFactoryName: dataFactory.name
      name: credential.name
      type: credential.type
      typeProperties: credential.typeProperties
    }
  }
]
